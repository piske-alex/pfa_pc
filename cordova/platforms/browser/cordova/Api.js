...
Api.prototype.addPlugin = function(pluginInfo, installOptions) {
  ...
  var projectFile =
    this._handler.parseProjectFile && this._handler.parseProjectFile(this.root);
...
Api.prototype._getUninstaller = function(type) {
  ...
  return function(item, plugin_dir, plugin_id, options, project) {
    ...
    if (!installer) {
      console.log(
        "browser plugin uninstall: unrecognized type, skipping : " + type,
      );
    } else {
      var wwwDest = options.usePlatformWww
        ? self.getPlatformInfo().locations.platformWww
  ...
  var modulesToInstall = plugin
    ...
    .map(function(moduleToInstall) {
      var moduleName =
        plugin.id +
        "." +
        (moduleToInstall.name || moduleToInstall.src.match(/([^\/]+)\.js/)[1]);
      var obj = {
